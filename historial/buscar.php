<?php 
    ...
    //echo $fecha;
    if($fecha !="0"){
        //$consulta = $link->query("SELECT id_entrada, id_salida FROM historial");
        ...
        INNER JOIN cliente AS c ON h.id_cliente = c.id_cliente WHERE h.fecha = '$fecha'
        UNION 
        ...
        INNER JOIN usuarios AS u ON h.id_user = u.id_usuario WHERE h.fecha = '$fecha'");
    }else{
        $consulta = $link->query("SELECT h.fecha, h.hora, h.id_entrada, h.id_salida, h.id_cedis, h.id_destino, u.nombre as nombre, c.nom_cliente FROM `historial` AS h 
        INNER JOIN usuarios AS u ON h.id_user = u.id_usuario 
